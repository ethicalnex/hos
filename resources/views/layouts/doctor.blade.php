...
<head>
    ...
    <title>@yield('title', 'Doctor Dashboard') - {{ auth()->user()->hospital->name }}</title>
...
<nav class="navbar navbar-expand-md navbar-dark shadow">
    <div class="container-fluid">
        <a class="navbar-brand" href="{{ route('doctor.dashboard') }}">
            <i class="fas fa-user-md me-2"></i>Dr. {{ auth()->user()->name }}
        </a>
...
<nav id="sidebar" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse" style="height: calc(100vh - 56px);">            <div class="position-sticky pt-3">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('doctor.lab.orders.*') ? 'active' : '' }} 
                           {{ !auth()->user()->hospital->hasFeature('lab') ? 'disabled' : '' }}"
                           href="{{ auth()->user()->hospital->hasFeature('lab') ? route('doctor.lab.orders.index') : '#' }}">
                            <i class="fas fa-flask me-2"></i>Lab Orders
                        </a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('doctor.emr.index') }}">
                            <i class="fas fa-procedures me-2"></i>EMR Records
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('doctor.prescriptions.*') ? 'active' : '' }}" 
