...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center py-3">
        <h1 class="h3 fw-bold text-primary">
            <i class="fas fa-hospital me-2"></i>{{ $hospital->name }} Payment Settings
        </h1>
        <a href="{{ route('super-admin.payment-settings.index') }}" class="btn btn-secondary">
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('super-admin.payment-settings.update', $hospital) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="public_key" class="form-control" 
                                   value="{{ $hospital->paymentSettings->where('payment_gateway', 'paystack')->first()?->public_key }}">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="webhook_secret" class="form-control"
                                   value="{{ $hospital->paymentSettings->where('payment_gateway', 'paystack')->first()?->webhook_secret }}">
                        </div>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('super-admin.payment-settings.update', $hospital) }}">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Public Key</label>
                            <input type="text" name="public_key" class="form-control"
                                   value="{{ $hospital->paymentSettings->where('payment_gateway', 'flutterwave')->first()?->public_key }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Secret Key</label>
                            <input type="password" name="secret_key" class="form-control">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Webhook Secret</label>
                            <input type="text" name="webhook_secret" class="form-control"
                                   value="{{ $hospital->paymentSettings->where('payment_gateway', 'flutterwave')->first()?->webhook_secret }}">
                        </div>
